...
한 턴마다 불이 퍼지고 상근이가 이동할 때,  
상근이가 보드 밖으로 최소 몇 번만에 나갈 수 있는지 구하는 문제

문제만 읽으면 전형적인 BFS 문제로 보인다.  
여기서 약간 헷갈릴 수 있는 부분이 있다.

1. 상근이가 이동하려는 자리에 불이 퍼질 때,  
   상근이는 이동할 수 없다.
2. 상근이가 있는 자리에 불이 퍼질 때,  
   상근이는 다른 위치로 이동할 수 있다.

위 두가지를 조심해야 한다.

위 두가지를 포함하여 문제를 풀기 위해  
먼저 불이 퍼지고 (1)  
그 다음에 상근이가 이동한다 (2)

> 불이 퍼질 때 마다  
> 보드에 표시(*)해줌으로써  
> 상근이는 다음으로 불이 퍼지는 위치를 알 수 있다.

> 상근이가 이동할 때는  
> 현재 위치에 불의 유무는 체크할 필요 없다.  
> (2번 조건 때문)

위처럼 처리해주기 위해  
불이 퍼질 때는 보드에 표시해주고,  
상근이가 이동할 때는 보드에 표시하지 않는다.

> 불은 직접 보드에 표시하기 때문에 별도의 visited 배열은 필요하지 않다.  
> 하지만 상근이는 visited 배열을 이용해야 한다.

그리고 매턴마다 불과 상근이가 한칸씩 이동해야 하기 때문에,  
현재 큐를 비워주면서 다음으로 이동하려는 칸을 별도의 큐에 넣어준다.  
마지막에 현재 큐를 다음 큐로 바꿔주면 된다.
...
)( 이런 케이스만 아니라면, 다른 괄호는 모두 변경해도 좋다.  
...
`시간복잡도:` $O(ElogE or ElogV)$

`어떻게 접근했는지, 풀었는지 설명:`

전력을 아끼기 위해 최대한 도로의 불을 꺼야 하는데,  
도시에 있는 모든 두 집 쌍에 대해, 불이 켜진 길만으로 서로를 왕래할 수 있어야 한다.  
-> 불을 켜야 하는 도로들의 쌍 중에서, MST를 구해야 한다.
...
> 이때 1-3은 불필요한 도로이다.
> 
> 결국 MST를 구했을 때의 값이 정답이 된다.

절약할 수 있는 최대 액수  
= 모든 도로의 길이의 합 - MST의 길이의 합

이 문제에서 V = 2백만, E = 2백만 이기 때문에  
프림 알고리즘과 크루스칼 알고리즘 모두 이용 가능하다.
...
기존 배낭 문제는 제한된 W(무게) 내에서 최대한의 V(가치)를 얻는 문제  
...
다행히 두 점 사이의 거리의 제곱과 반지름의 합의 제곱이 int 범위를 넘지 않는다.  
-> 루트를 씌우지 않고 비교했다.  
...
현재 위치에서 이전 위치에 있는 리스트의 값과 dp 값을 비교하여 현재 dp 값을 결정한다.

현재 위치 i에서 이전 위치 j를 비교한다.  
`dp[j] < dp[i] && list[j] >= dp[i]` - `dp[i] = dp[j]+1`

이때 모든 위치에서 dp를 비교하여 최댓값이 정답이 된다.

> 마지막 위치의 dp 값은 마지막 원소를 포함하는 경우에서 LIS의 길이를 저장한 것이다.  
> (전체 경우의 수를 포함하지 않는다)
...
`사용한 자료구조 & 알고리즘:` 백트래킹, 완전 탐색

`시간복잡도:` $O(8^8)$

`어떻게 접근했는지, 풀었는지 설명:`

왼쪽부터 순서대로 깨지지 않은 계란을 한번씩 쳐서  
깰 수 있는 계란의 최대 개수를 구하는 문제

인풋이 작은 점 (1),  
모든 경우의 수를 다 체크해야 하는 점 (2)  
위 두가지를 고려해서 완전 탐색으로 풀었다.

      여기서 내가 놓친 부분은  
      현재 들고 있는 계란을 무조건 한번만 쳐야 한다는 것이다.
      (한번 쳤다면, 다음 계란을 들어야 한다)

      나는 현재 들고 있는 계란이 깨지지 않았다면,
      계속 칠 수 있다고 생각했는데,
      이는 잘못된 접근이다.

      결국 전자의 규칙에 의해 시간복잡도가 줄어든다.
...
주어진 행성 간 거리 정보를 이용해  
모든 행성을 탐색하는 데의 최단 거리를 찾는 문제

특정 행성에서만 다른 행성으로 이동하는 것이 아니라,  
여러 행성에서 다른 행성으로 이동하기 때문에  
다익스트라, BFS로 풀기 어려운 문제라고 판단했다.

결국 모든 행성 간 최단 거리를 구해야 하기 때문에  
플로이드 워셜 알고리즘을 이용해야 한다.

그리고 모든 행성을 탐색하는데의 최단 거리를 구해야 하기 때문에  
백트래킹을 이용해 모든 경우의 수를 탐색해야 한다.

문제 푸는 과정
1. 플로이드 워셜 알고리즘을 이용해 모든 행성 간 최단 거리를 구한다.  
   O(N^3)
2. 백트래킹을 이용해 모든 행성을 탐색하는데의 최단 거리를 구한다.  
   O(N!)
...
상어들이 이동하는 로직 과정  
(상어가 위아래로 움직인다 가정)
1. 만약 속도 s가 2(R-1)보다 크다면,  
   s %= 2(R-1)  
   (2(R-1)만큼 이동하면 다시 원래 위치로 돌아오기 때문)
2. (상어가 위쪽으로 이동할 때)  
   r >= s 이면, {r-s,c} 가 다음으로 이동할 위치가 된다.  
   r < s 이면, {0,c}로 두고 s-=r 을 해준 다음, 2 or 3 과정을 반복한다.
3. (상어가 아래쪽으로 이동할 때)  
   r+s <= R-1 이면, {r+s,c} 가 다음으로 이동할 위치가 된다.  
   r+s >= R-1 이면, {R-1,c}로 두고 s-=R-r-1 을 해준 다음, 2 or 3 과정을 반복한다.

위 과정대로 수행하면 시간복잡도가 O(1)로 줄어든다.  
(1번에 의해 중복되는 이동이 없어지고, 2or3 은 최대 3번만 수행하면 되기 때문)

전체 시간복잡도:  
사람이 이동하는 횟수 * (현재 잡을 수 있는 상어 찾는 시간 + 모든 상어가 이동하는 시간)  
= C *(R + R * C) = O(R * C^2)

      이 문제에서 나오는 방향은 총 4가지 이므로,  
      굳이 모든 케이스를 포함하여 로직을 짤 필요가 없다.
      4가지를 전부 구현해도 된다.
...
`사용한 자료구조 & 알고리즘:` 다익스트라, 정렬

`시간복잡도:` $O(ElogE)$

`어떻게 접근했는지, 풀었는지 설명:`

1. 다익스트라 알고리즘을 이용해 지헌, 성하 위치의 최단거리 List를 구한다.
2. 두 List를 이용해 각 정점 별 정보에 대한 별도의 `List result`를 생성한다.  
   (정보: 정점 번호, 정점까지에서 지헌, 성하까지의 거리의 합, 지헌 거리, 성하 거리)
3. result를 우선순위에 따라 정렬한다.
4. result의 맨 첫번째 원소의 정점 번호가 정답이 된다.

이때 우선순위를 정하는 것이 매우 중요하다.  

> 이 우선순위 때문에 정답률이 17%밖에 안되었다.

아래 우선순위를 만족해야 한다.  
(번호가 낮을 수록 우선순위가 높다)
1. 현재 지헌, 성하가 있는 정점은 제외한다.
2. 지헌, 성하가 갈 수 있는 곳 중 최단 거리여야 한다.
3. 지헌 거리가 성하 거리보다 작거나 같아야 한다.
4. 지헌 거리가 작아야 한다.
5. 정점 번호가 작아야 한다.
      ...
      이때 내가 간과한 점이 있었다.
      1번의 최단 거리는 전체 정점 중 최단거리가 아닌,
      갈 수 있는 정점 중 최단거리를 의미한다.
      (지헌, 성하가 있는 정점은 제외)

      따라서 모든 정점을 순회하면서 최단거리를 따로 구해야 한다.
      (순서 1번의 두개의 List와는 별개로 구해야 한다.)

      그리고 간과한 점이 하나 더 있는데,
      최단 거리의 우선순위가 가장 높기 때문에 
      지헌, 성하 정점을 제외한 모든 정점을 돌면서 구해야 하는데,
      나는 우선순위 2번을 체크하면서 최단거리를 구했다.
      (이는 틀린 접근)
...
2-0 각 다리 배열을 MAX 값으로 초기화  
2-1 bfs로 섬의 가장자리를 탐색한다.  
다음 위치가 바다라면(board[nr][nc] == 0), 해당 방향으로 계속 이동하여 섬이 있는지 확인한다.  
2-1-1 섬이 없다면(배열 바깥으로 나갔다면), 다리를 만들 수 없다.  
2-1-2 섬이 있다면, 다리의 길이를 구하여 다리 배열을 업데이트한다.  
(모든 섬에 대해 반복한다)
...
> (그리고 mst를 오랜만에 풀어서 잊었음)
      ...
      이 문제는 2차원 배열로 주어진 섬의 정보를
      컨테이너로 바꿔 단순하게 만드는 것이 중요하다.
      (섬과 다리의 정보를 컨테이너로 정리하면, MST를 이용해 다리를 이을 때도 편리하다)
...
k가 0이라면, 그대로 탐색해서 탈출 가능한지 판단한다.  
k가 1이라면, 한개씩 모든 경우의 수를 탐색해서 탈출 가능한지 판단한다.

이때 타일의 종류에 따라서, 내가 이동하는 경로가 정해지는데,  
이를 메소드화 해서 구현했다.
...
윷놀이 판을 구현하고, 주사위를 굴려서 이동하는 문제

여기서 헷갈렸던 점은  
윷놀이 판을 구현하는 것이었다.

말이 겹치지 않는 지를 체크하기 위해  
각 라인을 따로 배열로 구현해야 했다.

> 처음 나는 윷놀이 판에서 갈라지는 부분만  
> 따로 배열로 구현했는데,  
> 이렇게 하면 말이 겹치는 부분을 처리하기가 어려웠다.  
> 
> 1,2,3 모두 4 를 가리키고 다음에 5가 있다면,    
> {1,4}, {2,4}, {3,4}, {4,5} 이렇게 구현해야 한다.  
> (처음에 나는 {1,4,5}, {2,4,5}, {3,4,5} 이렇게 구현했음 -> 5가 겹칠 수도 있다)
...
각 유닛이 목표 위치로 이동할 수 있는지를 구하는게 어려웠다. (귀찮았다)

`bool canMove(Loc& u, int tr, int tc)`  
u 유닛이 tr,tc 위치로 갈 수 있는지 반환하는 메서드

BFS를 이용했는데 여기서 살짝 응용했다.

> 기존 visited는 방문했는지/안했는지를 구분하기 위해  
> bool 타입을 많이 쓰는데,  
> 
> 이 문제에서는 int 타입을 쓰고,  
> 남은 이동력을 저장했다.
> 
> if (visited[nr][nc] < nextI) ok,  
> else break;
> 
> 만약 같은 위치에서 이동력이 높은 유닛, 낮은 유닛이 있을 때,  
> 높은 유닛이 타겟으로 움직이는데 실패했다면,  
> 낮은 유닛은 무조건 실패한다.  
> -> 그래서 높은 유닛의 이동력을 저장해서 낮은 유닛은 자동으로 종료하게 만들었다.

queue를 이용해 다음 위치를 탐색했는데 이때 체크해야 할 것들
1. 배열의 범위를 벗어나는지
2. 다음 위치에 세력이 다른 유닛이 있는지
3. 다음 위치가 갈 수 없는 곳인지
4. 남은 이동력이 0보다 떨어지는지
5. 다음 위치 주변에 세력이 다른 유닛이 있어서 교전이 발생하는지

이때 다음 위치가 타겟 위치이라면 5번에 해당해도 true를 반환한다.  
(교전을 해도 타겟에는 이동할 수 있기 때문에)

위 로직을 이용해 문제를 풀 수 있었다.

      이때 한번 놓친 부분이 있었다.
      
      타겟 위치에 유닛이 있다면, 세력이 같더라도, 무조건 false를 반환해야 하는데,
      나는 세력이 같다면 true를 반환하는 코드를 짰다.

      이때 세력이 0,1 값을 가지기 때문에, 이를 1,2 로 바꾸고  
      0은 아무도 없는 값으로 생각했다.
      (-> 항상 초깃값을 생각하자)
...
각 차례마다, 두 정점이 같은 집합에 속해있는지 체크한다.  
-> 만약 속해있다면, 사이클이 생긴 것이므로, 그 차례를 출력하고 종료한다.  
-> 속해있지 않다면, 두 정점을 Union 한다.
...
1. 2^q 크기의 격자들을 모두 시계방향으로 90도 회전한다.
2. 얼음이 3개 이상 인접해있지 않은 칸은 얼음의 양이 1 줄어든다.
3. 1,2 과정을 모든 격자에 대해 Q번 반복한다.
4. 남아있는 얼음의 합(1)과 가장 큰 덩어리가 차지한 칸의 개수(2)를 출력한다.

>

      문제를 푸는 것 자체는 문제가 없었지만
      시간 제한이 빡빡했다.
      (처음엔 시간초과가 났다)

      그 이유는 rotate 함수를 호출할 때마다 
      새로운 배열을 생성하는 것이 시간이 오래 걸렸기 때문이다.

      그래서 rotate에 필요한 배열을 미리 생성해놓고, 그 배열을 사용하도록 수정했다.
      (rotate에서 사용하는 배열은 항상 모든 칸이 새로 업데이트 되어야 하기 때문에)

      -> 변수를 생성하는 비용도 생각하자

> 이 문제에서 배열을 생성하고 나서 rotate를 해줘도 괜찮은 이유  
> 
> 다른 문제와 달리 이 문제는,  
> 2차원 배열을 N개의 블록으로 나눴을 때  
> 각 블록마다 rotate를 해줘야 했다.  
> 
> 즉 2차원 배열의 모든 위치를 업데이트 하기 위해 총 N 번의 rotate를 하는데,  
> 이때 각 rotate는 각각의 위치에만 영향을 미치기 때문에,  
> N번의 호출 동안 하나의 배열만 이용해도 문제가 발생하지 않는다.
> 
> 한 배열만 가지고 N번의 rotate를 하는 것은 이 문제에만 해당하는 것이다.
...
`사용한 자료구조 & 알고리즘:` 분리 집합, 배낭 문제

`시간복잡도:` $O(NK)$

`어떻게 접근했는지, 풀었는지 설명:`

K명 미만의 사람들의 사탕을 빼았을 때, 뺐을 수 있는 사탕의 최댓값을 찾는 문제  
여기서 사람들은 친구끼리 다니는데, 특정 사람의 사탕을 뺏으면, 그 사람의 다른 친구들의 사탕도 전부 빼았는다.

특정 원소를 고르면, 그 원소와 관계가 있는 모든 원소도 뽑힌다.  
-> 해당 원소가 어떤 원소와 관계가 있는지 알아야 한다.  
-> 분리 집합

분리 집합을 이용하여 각 그룹의 인원 수, 사탕의 개수를 알 수 있다.

> 그룹의 정보를 얻는 방법
> 
> 모든 친구들의 관계를 이용해 union 작업을 할 때  
> b -> a 형태로 이어진다면, b의 정보를 a에 더해줘야 한다.
> 
> b 그룹 인원 2명, 사탕 3개  
> a 그룹 인원 3명, 사탕 4개  
> b -> a로 이어진다면,  
> a 그룹 인원 5명, 사탕 7개가 된다.
>
> union 작업을 한 뒤에  
> parent[i] == i인 사람의 정보가 해당 그룹의 정보가 된다.

해당 그룹들의 정보를 얻으면  
각 그룹의 인원 수, 사탕의 개수를 이용하여  
제한된 인원 내에서 최대 가치를 얻어야 한다.  
-> 제한된 무게에서 최대 가치를 얻는 문제  
-> 배낭 문제

최대 무게 K <= 3000, 최대 인원 N <= 30000  
O(NK)로 빡빡하게 시간 내에 풀 수 있다.

      union-find 작업 중  
      두 원소의 부모가 같은 경우를 예외 처리하지 않아서 틀렸다.
      (부모가 같은 경우는 무시해야 한다)
...
여기서 인풋을 저장할 때,  
학생 별 선호 학생을 저장하기 위한 `List<List<Integer>>` 컨테이너와  
학생 배치 순서를 저장하기 위한 `List<Integer>` 컨테이너를 사용했다.

> 학생 배치 순서는 순서대로 저장해야 하지만,  
> 선호 학생은 번호대로 저장해야 한다.
...
핵심
1. 구슬들을 2차원 배열이 아닌 컨테이너로 관리한다.  
   (컨테이너로 관리하면 구슬을 제거하고 나서, 압축하는 과정이 사라진다)
2. 마법, 폭발로 인해 구슬을 제거할 때  
   뒷 부분부터 제거해야 한다.  
   (앞 부분을 제거하면, 뒷 부분의 인덱스가 변경되어 오류가 발생한다)

> 

      구슬이 회오리 형태로 압축되기 때문에
      회오리 모양의 보드 번호를 만드는 작업이 오래걸렸다.
      -> 당황하지 말고, 규칙을 찾고 구현하자

      컨테이너의 원소들을 제거하는 작업은
      뒷부분 부터 해야 함을 기억하자
   
      컨테이너에서 연속된 특정 원소들을 찾는 것은
      subList를 이용하면 쉽게 할 수 있다.
...
맨 처음에 inst들을 일단 map에 저장해놓는다. // {폴더 이름, inst 정보}

그 다음 queue를 이용하여 폴더를 추가해줬다.  
루트 폴더인 main 폴더 먼저 push 해주고,  
map으로부터 main 폴더에 추가해줄 파일 & 디렉토리를 추가해주고,  
해당 폴더를 상위 폴더로 가지는 폴더들을 queue에 push 해준다.  
위 과정을 반복하면서 폴더 트리를 구성했다.

> 이때 inst들을 저장할 때, 폴더 이름이 겹칠 수 있다.
> 
> 나는 이를 해결하기 위해, value로 vector를 이용했지만,  
> multimap을 이용하면 더 쉽게 해결할 수 있을 것 같다.  
> (이때는 equal_range를 이용하면 된다)
...
> 위 split 함수를 구현하려면,  
> #include <regex> 를 해줘야 한다.

      여기서 새로 안 사실들
      regax를 이용한 spliㅅt 매서드 구현
...
여기서 현재 위치의 빌딩 i의 양쪽으로 보이는 경우를 모두 체크해야 하기 때문에,  
순회를 순방향 역방향 두번 돈다.

그리고 가장 가까운 거리에 있는 빌딩을 체크하기 위해  
distance를 따로 저장한다.  
(이때 거리가 같은 빌딩 중 번호가 작은 빌딩을 골라야 하기 때문에 역방향 -> 순방향 순서로 순회를 돈다)  
(우선순위 부여)
...
1. 여행 기록대로 움직이면서, 와드를 놓는 위치 값을 q.push() 해준다.
2. q.pop()을 하면서, 와드를 놓은 곳을 bfs로 영역을 체크한다.
3. 마지막 위치도 체크한다.
4. 결과 출력
...
적용되는 최적해:  
`4개의 스택들의 원소들 중에 현재 값보다 작고 가장 큰 자리에 현재 원소를 추가한다`

4개의 스택을 이용해 정렬하려고 한다면,  
모든 원소를 스택들에 넣었을 때  
스택들 내의 원소가 내림차순으로 정렬되어 있어야 한다.

이를 위해 매번 원소를 스택에 넣을 때,  
현재 스택들 중 가장 현재값보다 작고 가장 큰 스택에 넣어야 한다.
...
N개의 과일이 꽂혀있는 탕후루에서  
앞부분이나 끝부분의 과일들을 빼서  
과일의 종류를 2개 이하로 만들어야 한다.  
이떄 과일의 개수의 최댓값을 구하는 문제

이때 앞부분의 포인터와 끝부분의 포인터를 보고 접근하는 것은 힘들다고 판단했다.  
(현재 위치만 보고 최댓값을 찾는 건 힘들다고 판단했다)

이 문제는 이렇게 변형할 수 있다.  
N개의 원소 중에서, 종류가 2개 이하인  
연속한 부분수열을 찾는 문제
...
만약 현재 숫자의 종류가 2개 이하라면 r++ 해준다.  
2개 초과라면 l++ 해준다.

> 현재 숫자의 종류를 바로 확인할 수 있도록  
> Map 컨테이너를 사용했다.
