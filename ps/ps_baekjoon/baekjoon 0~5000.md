...
주어진 N을 이용해 배열에 회오리 모양으로 숫자를 채워서 출력하는 문제

맨 처음 시작점은 (N-1)/2, (N-1)/2 이다.
...
왼쪽으로 k번,  
위쪽으로 k번  
위 과정을 반복하면서 숫자를 채워나간다.  
(k는 위 과정을 한번 반복할 때마다 2씩 증가)

> 위 과정을 for문으로 구현하기 때문에,  
> 각 for문이 끝난 후  
> 다음 for문 전, 첫 위치로 이동해줘야 한다.

각 숫자가 어떤 위치에 있는지 저장하기 위해  
추가로 Map을 이용해 위치를 저장했다.
...
`사용한 자료구조 & 알고리즘:` 백트래킹, BFS

`시간복잡도:` $O(25C7)$

`어떻게 접근했는지, 풀었는지 설명:`

5*5 보드 위에서 상하좌우로 인접한 7개의 칸을 선택하는데,  
이때 S가 4개 이상인 경우의 수를 구하는 문제

> 처음엔 BFS로 접근하려고 했지만  
> 경우의 수가 너무 많아서 못한다고 판단했다.

칸의 개수가 25개밖에 없으므로  
모든 경우의 수를 탐색해도 된다고 판단했다.

푸는 과정
1. 25개의 칸 중 7칸을 선택한다.
2. 해당 7칸에서 S가 4개 이상인지 확인한다.  
   (미만이라면 다시 1번으로)
3. 해당 7칸이 인접한지 확인한다.  
   (BFS를 사용한다)
4. 2,3번을 통과했다면 result++
5. 1번의 모든 경우의 수를 체크한 뒤 result를 출력한다.



      여기서 BFS를 이용할 때,
      기존의 7칸이 있는지 확인이 필요한데
      각 칸의 좌표값을 별도의 클래스로 관리하면
      equals() & hashcode() 메서드를 오버라이딩해야 한다.
      (위 메서드를 재정의 하지 않으면 객체 내부 값이 같아도 다르다고 판단할 수 있기 때문)

      위 과정이 매우 번거롭기 때문에
      이런 별도의 클래스를 이용해 Map이나 Set의 키로 사용할 때는
      재정의보다 내가 직접 Integer나 String으로 변환해서 사용하는 것이 편리하다.
      (Integer, String은 equals() & hashcode() 메서드가 재정의 되어 있기 때문)
...
K명의 노드를 뽑는데, 해당 노드들이 모두 연결되어야 한다.  
위 케이스에 해당하는 노드를 번호가 작은 순서대로 출력하는 문제

> 만약 1-2, 2-3 일 때 1-3이 인정된다면  
> 유니온-파인드를 이용하면 쉽게 풀리지만,  
> (각 그룹별 인원 수를 저장하고, 연결되어 있는지 확인하면 가능)  
> 이 문제는 1-3이 인정되지 않는다.  
> (1-2, 2-3, 1-3 이 모두 있어야 1,2,3이 연결된 것으로 인정)
> 
> 따라서 일일이 연결되어 있는지 확인해야 한다.

1. 입력을 받아 edge를 2차원 배열로 저장한다.
2. edge의 size가 K 이상인 노드들을 뽑는다.
3. 2번을 통해 뽑은 노드들 중 K개를 뽑는다.  
   이때, 해당 노드들이 모두 연결되어 있는지 확인한다.  
   (직접 2차원 배열을 탐색하며 연결되어 있는지 확인)

이론적으로 edge의 개수는 최대 5600이기 때문에  
edge의 size가 K 이상인 노드들의 최대 개수는 약 150개 정도이다.  
(150C2 = 11250 => 5600 * 2 와 비슷한 수준)

즉 worst case의 시간복잡도는  
`O(150Ck * k^2)` 정도이다.  
(노드를 선택하는 시간 * 노드들이 연결되어 있는지 확인하는 시간)

이론상으론 무조건 시간 초과이지만,  
오름차순으로 탐색하여 중간에 끝낼 수 있기 때문에  
실제로는 시간 초과가 나지 않는다.
...
주어진 막대 기둥을 이용해 가장 작은 다각형을 만들어야 한다.  
이때 중간에 오목하게 들어간 부분이 있으면 안된다.  
-> 제일 높은 부분을 기준으로 양쪽으로 점점 높이가 낮아지는 다각형을 만들어야 한다.

1. 높이가 가장 높은 기둥을 찾는다.
2. 해당 기둥을 기준으로 왼쪽 끝/오른쪽 끝에서 시작해서  
   높이가 가장 높은 기둥까지의 면적을 구한다.  
   (높이가 높아질 때만 면적을 구하면 된다)
3. 높이가 가장 높은 기둥의 면적을 구한다.

2와 3의 면적을 더하면 정답이 된다.
...
적용되는 최적해:  
이전 원소까지 측정 가능한 구간이 [a,b]라면  
이전 원소들과 현재 원소 e를 이용해 [a+e,b+e] 구간을 측정할 수 있다.

> 진짜 미친 접근이다.

1. 입력을 오름차순으로 정렬한다.
2. 첫번째 원소부터 시작해서, 현재 원소를 이용해 측정 가능한 구간을 갱신한다.  
   (맨 처음 원소를 접근하기 전, 측정 가능한 구간은 [0,0]이다)
3. 2번 진행 중, 만약 이전 구간과 새로운 구간에서 끊기는 부분이 있다면,  
   이전 구간의 끝 + 1부터 측정이 불가능하다.  
   -> 이전 구간의 끝 + 1이 답이 된다.

> 이전 구간 [1,2]이고, 현재 원소까지 이용한 새로운 구간이 [3,4] 라면  
> 현재 원소까지 이용해서 구할 수 있는 전체 구간은 [1,4]가 된다.  
> (현재는 정수만 측정하기 때문에 2와 3이 이어진다)
> 
> 만약 [1,2]이고, [4,5]라면  
> 3은 측정이 불가능하다.

    이 문제를 풀면서,  
    이전 원소까지 측정 가능한 구간을 이용해  
    현재 원소를 이용해 측정 가능한 구간을 갱신하는 방법을 생각하지 못했다.
...
N개의 학생 중 자신의 키가 몇번째인 지 정확히 아는 사람의 수를 반환하는 문제
...
> 일 때
> 
> 1과 3의 관계를 위상 정렬을 통해 알기 힘들다.

여기서 특징을 알게 됐다.  
`i번째 학생이 키가 몇번째인 지 알고 있다.`  
-> `forawd/backward로 탐색 했을 때 모든 노드를 탐색할 수 있다`

따라서 모든 학생이 BFS를 이용해  
키의 순서를 알 수 있는지 순회하면 결과를 알 수 있다.
...
`사용한 자료구조 & 알고리즘:` 정렬, DP, 이분 탐색

`시간복잡도:` $O()$

`어떻게 접근했는지, 풀었는지 설명:`

먼저 높이의 내림차순으로 그림들을 정렬한다.

적용되는 dp:  
`dp[i] = max(dp[i+1], dp[k])`  
(k = i 그림의 높이-S 보다 작은 그림들 중에 가장 높이가 높은 그림의 인덱스)

> k는 이분 탐색을 이용하여 찾는다.

`getMax(int i)` - 현재 그림에 해당하는 높이가 최대일 때의 최대 가치를 반환한다.  
1. `getMax(i+1)` - 현재 그림을 포함하지 않을 때의 최대 가치를 구한다.  
   (바로 다음 인덱스에서의 최대 가치에 해당한다.)
2. 이분 탐색으로 현재 그림의 높이 h-S 보다 작은 그림들 중에 가장 높은 그림의 인덱스 k를 찾는다.
3. `getMax(k) + 현재 그림의 가치` - 현재 그림과 k번째 그림을 놓으면 현재 그림의 가치를 얻는다.  
   -> 현재 그림의 가치를 포함하는 최대 가치를 구한다.
4. 1,3번의 최댓값을 반환한다.
...
`사용한 자료구조 & 알고리즘:` 슬라이딩 윈도우

`시간복잡도:` $O(N)$

`어떻게 접근했는지, 풀었는지 설명:`

회전 초밥 정보와 추가 쿠폰 정보가 주어졌을 때,  
최대로 먹을 수 있는 초밥의 종류를 구하는 문제

K개만큼 연속으로 초밥을 먹기 때문에  
슬라이딩 윈도우를 이용하면 편하게 구할 수 있다.  
그리고 종류를 바로 알 수 있게 Map을 사용했다.

```
Map<Integer, Integer> cur; // 현재 k개의 초밥 정보가 들어 있음 {초밥 번호, 개수}
for(int i=0;i<N;i++){
   if(쿠폰이 cur에 없다) result = max(result, cur.size()+1));
   else result = max(result, cur.size()+1));
...
```
      ...
      여기서 행사에 무조건 참여한다는 조항을 까먹었다.
      그래서 특정 경우엔 (쿠폰 초밥이 벨드에 없고, 벨트에 있는 초밥의 종류가 K보다 작을 때)
      벨트에 있는 초밥의 종류 s 를 반환하려 했는데,
      (최대한 많은 종류의 초밥을 연속하지 않아도 먹으려고 하는 경우에 해당한다)

      무조건 행사에 참여한다고 가정하기 때문에
      k개를 연속으로 먹어야 한다.
      -> 이를 까먹어 처음에 틀렸다.
...
각 음악의 순서를 입력받고  
그 순서들을 이용해 위상 정렬을 통해 순서를 정한다.

이때 답이 없는 경우에는 0을 출력해야 하는데,  
이를 판단하기 위해 위상 정렬을 하고 나서,   
결과 배열의 크기가 n이 아니라면 0을 출력한다.

> 1 5 3 순서를 입력받으면,  
> 1 -> 5, 5 -> 3의 관계를 만들어 위상 정렬을 한다.  
> (degree는 5, 3이 1씩 증가)
> 
> 위 케이스에서 3에 대해 1과 5를 둘 다 순서를 추가해줄 수 있지만,  
> 이미 1 -> 5 순서가 있으므로 이는 무의미하다.  
> (굳이 안해줘도 무방하다)

위상 정렬 과정  
1. degree가 0인 정점을 큐에 넣는다.
2. 큐에서 정점을 꺼내고, 해당 정점과 연결된 정점들의 degree를 1씩 감소시킨다.
3. degree가 0이 된 정점을 큐에 넣는다.
4. 큐가 빌 때까지 2, 3을 반복한다.
5. 큐가 비었을 때, 결과 배열의 크기가 n이 아니라면 0을 출력한다.
6. 결과 배열을 출력한다.
...
재료의 정보를 가지고  
완제품을 만들기 위해 필요한 기본 재료의 개수를 구하는 문제

여기서 핵심은  
어떤 재료는 기본 재료일 수 있고,  
어떤 재료는 기본 재료가 아닐 수 있다.  
(기본 재료가 아니다 -> 이 재료를 만들기 위해 다른 재료가 필요하다)
...
1. 위상 정렬을 이용해 기본 재료를 찾는다.  
   (차수가 0인 재료들을 찾는다)
2. 기본 재료들을 이용해 다른 재료들을 만들어 나간다.  
   (기본 재료들을 이용해 다른 재료들을 만들어 나가면서,  
   필요한 기본 재료의 개수를 구한다)
3. 위 과정을 통해 완제품을 만들기 위해 필요한 기본 재료의 개수를 구한다.

2번 과정 중  
현재 재료를 만들기 위해  
필요한 재료들이 기본 재료일 수 있고, 아닐 수도 있다.  
이를 구분하기 위해  
재료들이 저장된 해시를 이용했다.

      하나 놓친 점은
      재료의 정보를 해시에 저장해서 사용했는데,
      해시는 순서가 없다.
      마지막 출력은 순서가 있어야 한다.
      이를 놓쳐서 한번 틀렸다.
...
문자열이 입력으로 들어오면,  
해당 문자열이 잠수함 정규식에 맞는지 판별하는 문제

잠수함 정규식: `(100~1~|01)~`

1. 끝이 0일 때  
   1-1 다음 문자가 0이라면 1번 다시 검증  
   1-2 다음 문자가 1이라면 2번 검증
2. 끝이 1일 때  
   2-1 다음 문자가 100이라면 1번 검증  
   2-2 다음 문자가 1이라면 2번 다시 검증  
   2-3 다음 문자가 01이라면 3번 검증
3. 끝이 01일 때  
   3-1 다음 문자가 100이라면 1번 검증  
   3-2 다음 문자가 01이라면 3번 다시 검증

`isSubmarine(i,f)`:  
i번째 위치에서 이전 문자열이 f번째 문자열일 때,  
해당 문자열이 잠수함 정규식에 맞는지 판별하는 함수

맨 처음에는 3번으로 시작해야 한다.  
(100으로 시작하는지, 01로 시작하는지 판별)

그리고 마지막에는 1이나 01로 끝나야 한다.  
(0으로 끝나면 false 반환)
...
모든 원소를 순회하면서 해당 번호의 줄에서,  
프렛을 탐색해야 한다.

스택을 이용한다.  
모든 기타줄마다 스택을 가지고 있다.

while(현재 프렛 < 해당 기타줄의 프렛)
   기타줄의 프렛 pop; result++;
if(현재 프렛 != 현재 기타줄의 프렛)
   기타줄의 프렛 push; result++;
...
`사용한 자료구조 & 알고리즘:` 두 포인터, 정렬

`시간복잡도:` $O(N^2)$

`어떻게 접근했는지, 풀었는지 설명:`

세 숫자를 더했을 떄 0이 되는 경우의 수를 구하는 문제

완전 탐색으로 접근하면 O(N^3)이 나온다.

여기서 정렬을 하고 투 포인터를 이용하면 O(N^2)으로 줄일 수 있다.

정렬 이후,  
0~N-1의 원소 i를 고정하고,  
`l=i+1`, `r=N-1` 으로 시작하여  
`arr[l]+arr[r] == -arr[i]`인 경우를 찾는다.

> `arr[l]+arr[r]+arr[i] > 0`인 경우  
> 전체 값을 줄여야 한다 -> r -= 1
> 
> `arr[l]+arr[r]+arr[i] < 0`인 경우  
> 전체 값을 늘려야 한다 -> l += 1
      ...
      여기서 한 숫자를 고정한 이후의 문제는 
      3273 문제(두 수의 합)와 같아진다.

      나는 처음에 처음부터 투 포인터로 두 숫자를 정하고,
      이분 탐색으로 나머지 숫자를 찾으려고 했다.
      이때 투 포인터 l,r의 다음 위치를 정하는 것이 불가능했다.
