...
int, long long 같은 기본 타입을 비롯해,  
string 타입도 비교 연산자가 구현되어 있다.

이때 일반 숫자 타입과 string 타입의 비교 연산자는  
로직이 살짝 다르다.

1. 두 숫자(문자열)의 길이가 같다면  
   숫자 타입의 비교와 string 타입의 비교는 같은 결과를 낸다.
2. 두 숫자(문자열)의 길이가 다르다면  
   숫자 타입의 비교는 길이가 긴 숫자가 더 크다고 판단하지만,  
   string 타입의 비교는 사전 순으로 판단한다.

> "21" vs "3" -> "3"이 더 크다.  
> 하지만  
> 21 vs 3 -> 21이 더 크다.

> 만약 비교할 두 대상의 길이가 같다면  
> string 타입으로 비교해도 된다.  
> (물론 시간복잡도는 string 타입이 더 높다 - O(N))  
> (대신 더 긴 숫자를 비교할 때는 string 타입으로 비교해야 한다)
