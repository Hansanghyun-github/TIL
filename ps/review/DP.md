...
여기서 배낭 문제가 일반적인 DP 문제와 구별되는 점은,  
각 물건을 넣을지 말지를 결정하는 문제이다.  
(즉, 각 물건을 넣을지 말지를 결정하는 문제이다)

> 한번 넣은 물건은 다시 넣을 수 없다.

이를 감안하여 점화식이 다음과 같이 정의된다.  
...
> 배낭 문제의 시간복잡도는 아이템의 개수 x 무게의 최대값
