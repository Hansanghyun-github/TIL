...
최적으로 제한 시간 내에 퍼즐을 모두 해결하기 위한 숙련도의 최솟값을 구하는 방법은  
아직까진 모르겠다.

모든 경우의 수를 탐색해야 하는데,  
숙련도의 범위는 1에서 100000 사이이기 때문에  
이분탐색, 파라메트릭 서치를 이용해야 한다.

1. l과 r을 이용해 mid 구한다.
2. 구한 mid를 이용해 퍼즐을 제한 시간 내에 모두 해결 가능한지 체크 - O(n)
3. 2번에 결과에 따라 answer, l, r 을 업데이트
