...
- A 레코드 : 도메인 네임을 IP 주소로 매핑한다. (정방향 조회)
- CNAME 레코드 : 도메인 이름을 다른 도메인 이름으로 매핑한다.
- NS 레코드 : 도메인 이름을 관리하는 DNS 서버를 식별한다.
- PTR 레코드 : IP 주소를 도메인 이름으로 매핑한다. (역방향 조회)

> 각 레코드는 A -> B 형태이다.  
> (이는 B -> A를 보장하지 않는다)  
> (A 레코드와 PTR 레코드는 독립적인 레코드이다)
> 
> 실제로 A 레코드를 이용해 도메인 네임에 해당하는 IP 주소를 찾아도,  
> PTR 레코드를 이용해 IP 주소에 해당하는 도메인 네임이 A 레코드의 결과와 일치하지 않을 수 있다.
...
> 포트번호 안쓰면 TCP 80(HTTP), 443(HTTPS)로 간주
...
web에서 기본적인 두가지기 위해 설계되었다. & TCP/IP를 활용
