이 글은 API 서버에서 처리하는 Main API의 성능 개선 과정을 다룬다.
...
Main API는 사용자가 로그인 한 뒤에 항상 호출되는 API이다.  

> 스핀로그에서 가장 많이, 자주 호출되는 API 이다.

따라서 Main API의 성능이 중요하다.

---

## 기존 Main API 부하 테스트 결과

> nGrinder를 이용해 100명의 vuser가 20분 동안 Main API를 호출했다.
...
Main API에서 한달치 articles와 오늘 articles를 가져오는 작업을 수행할 때,  
