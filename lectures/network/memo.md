...
1972 - 컴퓨터 바이러스의 개념이 처음 등장  
1984 - 프레드 코헨이 컴퓨터 바이러스의 개념 정립  
1986 - 최초의 바이러스 `브레인 바이러스`  
1988 - 최초의 웜 `모리스 웜`  
1999 - 매크로 바이러스 `멜리사 바이러스` 출현  
2001 - 웜에 의한 대규모 피해 발생(코드 레드)  
...
1세대 원시형 바이러스
1. 부트 바이러스: 플로피 디스크나 하드 디스크의 부트 섹터에 감염되는 바이러스, 부팅할 때 자동으로 동작->모든 프로그램 감염
	브레인, 몽키, 미켈란젤로 바이러스
	> 시스템의 부팅 순서  
	> POST -> CMOS 로드 -> MBR 로드(이때 부트 바이러스 감염) -> 운영체제 정보 로드
2. 파일 바이러스: 파일을 직접 감염시키는 바이러스(COM, EXE, 오버레이 파일)(최초의 파일 바이러스 - 예루살렘 바이러스)
	ex) 썬데이, 스콜피온, 크로우, FCL, CIH

![Alt text](<Pasted image 20231018191844.png>)

2세대 암호형 바이러스
- 바이러스 코드를 쉽게 파악하고 제거할 수 없도록 암호화된 바이러스  
ex) 슬로우, 캐스케이드, 원더러, 버글러 바이러스
...
ex) 브레인, 조시, 512, 4096 바이러스

4세대 다형성 바이러스
- 백신 프로그램이 특정 식별자를 이용하여 바이러스를 진단하는 기능을 우회하기 위해 만들어진 바이러스
- 코드조합을 다양하게 할 수 있는 조합 프로그램을 암호형 바이러스에 덧붙여 감염 
	실행될 때마다 바이러스 코드 자체를 변경시켜 식별자를 통해 구분하기 어렵게 함
- 코드 조합을 다양하게 할 수 있는 조합 프로그램을 암호형 바이러스에 덧붙인 바이러스 -> 백신 프로그램이 특정 식별자를 이용하여 바이러스를 진단하는 기능을 우회할 수 있다

5세대 매크로 바이러스
- 응용프로그램 및 사무용 관련 프로그램이 개발되면서 스크립트 형태의 실행 환경을 이용하여 전파되는 바이러스. 주로 MS 오피스 프로그램의 매크로 기능을 이용하여 감염
- 엑셀 또는 워드와 같은 문서 파일의 매크로 기능을 이용하기 때문에 워드나 엑셀 파일을 열 때 감염됨
ex) 워드 컨셉트, 와쭈, 액셀-라룩스 바이러스

웜의 의미
- 인터넷 또는 네트워크를 통해서 컴퓨터에서 컴퓨터로 전파되는 프로그램. 
...
    베이글, 넷스카이, 두마루, 소빅
2. 시스템 공격형 웜: 운영체제 고유의 취약점을 통해 내부 정보를 파괴 혹은 컴퓨터를 사용할 수 없는 상태로 만들거나 외부 공격자가 시스템 내부에 접속할 수 있도록 백도어를 설치하는 웜
    아고봇, 블래스터, 웰치아
3. 네트워크 공격형 웜:특정 네트워크나 시스템에 대해 서비스 거부 공격을 수행한다.
    져봇, 클레즈

백도어와 트로이목마
- 백도어란 정상적인 인증 과정을 거치지 않고 운영체제나 프로그램 등에 접근할 수 있도록 만든 일종의 통로이다. 
- 트로이목마란 ==사용자가 의도하지 않은 코드==를 정상적인 프로그램에 삽입한 형태
...
전치법 - 단순히 메시지의 문자 위치를 바꾸는 방법

BC 400년 - 암호문과 봉의 굵기를 알려주면, 암호문을 봉에 감아서 해석하는 방식<br>
-> 봉의 굵기가 암호화 키

대체법: 해당 글자를 다른 글자로 대체하는 방법
> 단일치환  
...
> 충분히 큰 두 소수의 곱에서 두 소수를 찾기 어려운 점을 이용함
