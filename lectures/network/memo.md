...
`IDS의 주요 기능`  
1. 데이터의 수집
- HIDS는 운영체제에 부가적으로 설치되어 운용되거나 일반 클라이언트에 설치(네트워크에 대한 침입탐지는 불가능, 스스로가 공격 대상이 될 때만 침입 탐지 가능)
- NIDS는 네트워크에서 하나의 독립된 시스템으로 운용(감시 영역이 상대적으로 매우 크다)
...
(정보 보안 거버넌스의 구현 요건은 안 봐도 됨 - 안 나옴)
...
시스템 운영팀 밑에 보안 인력이 있으면, 발언권이 떨어진다.

IT 기획팀 밑에 보안 인력이 있으면, 전보다 발언권은 높아졌다.
