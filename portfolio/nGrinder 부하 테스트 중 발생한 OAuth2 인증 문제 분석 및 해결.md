...
> 요청을 보내는 스레드: A, B  
> A 스레드는 API 서버에 요청을 보내고,  
> B 스레드는 모의 서버에 요청을 보낸다.
> 
...
sequenceDiagram
   ...
   participant HttpRequest as HttpRequest 객체
...
> 실제 쿠키 검증 로직은 `HttpRequest` 객체 내부에서 이루어진다.  
...
sequenceDiagram
   ...
   COOKIE_STORE->>VUser: 쿠키 누락

   VUser->>API: 인증 코드를 이용해 API 서버에 인증 요청 (쿠키 누락)
   API-->>VUser: 인증 실패
```
